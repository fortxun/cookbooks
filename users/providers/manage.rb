...
action :remove do
  ...
  elsif not search(:node, "name:#{node[:users][:node_filter]}").any?{|a| a.name == "#{Chef::Config[:node_name]}"}
    Chef::Log.warn("This recipe is only for nodes returned by node:name search node_filter.")
  else
...
action :create do
  ...
  if Chef::Config[:solo] and not chef_solo_search_installed?
    Chef::Log.warn("This recipe uses search. Chef Solo does not support search unless you install the chef-solo-search cookbook.")
  #If the this node is not in the list returned by the search using node_filter, bale out
  elsif not search(:node, "name:#{node[:users][:node_filter]}").any?{|a| a.name == "#{Chef::Config[:node_name]}"}
    Chef::Log.warn("This recipe is only for nodes returned by node:name search node_filter.")
  else
